...
#import "PCPassengerSynchroViewController.h"
...
@interface PCPassengerSynchroViewController ()<QMapViewDelegate,TLSPassengerManagerDelegate,TencentLBSLocationManagerDelegate>
...
@implementation PCPassengerSynchroViewController

UIColor *PCColorWithHex(long hex) {
...
// 获取导航SDK默认路况颜色
UIColor *PCRouteTrafficStatusColor(long trafficDataStatus) {
    
    switch (trafficDataStatus)
    {
        case 0:
        {
            return PCColorWithHex(0x05B473);
            break;
        }
        case 1:
        {
            return PCColorWithHex(0xFABB11);
            break;
        }
        case 2:
        {
            return PCColorWithHex(0xE61C3F);
            break;
        }
        case 3:
        {
            return PCColorWithHex(0x6ca4f2);
            break;
        }
        case 4:
        {
            return PCColorWithHex(0x932632);
            break;
        }
        default:
        {
            return PCColorWithHex(0xc2c2c2);
            break;
        }
    }
}
...
- (void)setupPassengerManager
{
    ...
    self.passengerManager.pOrderID = kSynchroPassenger2OrderID;
    self.passengerManager.orderType = TLSBOrderTypeRidesharing;
...
- (void)tlsPassengerManager:(TLSPassengerManager *)passengerManager didFetchedData:(TLSPFetchedData *)fetchedData
{
    ...
    if (fetchedData.hasArrived) {
        // 已经到达上车点或下车点
        self.remainingString = [NSString stringWithFormat:@"已到达"];
...
- (NSArray<QSegmentColor *> *)getSegmentColorsWithItems:(NSArray<TLSBRouteTrafficItem *> *)items
{
    ...
    for(int i = 0;i < items.count; ++i)
    {
        ...
        routeStyle.color = PCRouteTrafficStatusColor(item.color);
